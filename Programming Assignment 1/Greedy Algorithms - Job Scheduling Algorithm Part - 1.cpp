...
int main(){
    ...
    freopen("/home/ankit/Desktop/Coursera Courses/Design and Analysis of Algorithms - Part 2/Programming Assignment 1/Question-1-Answer-Greedy-Difference.txt" , "wb" , stdout);
    ...
    // this vector stores the difference (wj - lj) of the jobs provided in the input
    vector< pair< int , pair< int , int > > > greedyVector;

    for(int i = 0 ; i < SZ( jobs ) ; ++i){
        greedyVector.PB( make_pair( jobs[i].first - jobs[i].second , make_pair( jobs[i].first , jobs[i].second ) ) );
    }
    ...
    // these help in the evaluation of the weighted sum and the completion times of the jobs
    ULL weightSum = 0;
